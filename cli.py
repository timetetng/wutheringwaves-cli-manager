...
def update():
    ...
    if shutil.which("pacman"):
        ...
        if check_aur.returncode == 0:
            typer.secho("检测到通过 AUR 安装，尝试使用 yay 自动更新...", fg="cyan")
            if shutil.which("yay"):
                try:
                    subprocess.run(["yay", "-S", "ww-manager", "--noconfirm"], check=True)
                ...
                typer.secho("未找到 yay，请手动更新 AUR 软件包。", fg="yellow")
                return

    if shutil.which("uv"):
        typer.echo("正在检查 PyPI 更新...")
        try:
            subprocess.run(["uv", "tool", "upgrade", "ww-manager"], check=True)
            typer.secho("正在更新中。", fg="green")
        except subprocess.CalledProcessError as e:
            typer.secho(f"uv 更新失败: {e}。\n请手动执行 uv tool upgrade ww-manager 更新", fg="red")
    else:
